          ...
          <div class="box">
            ...
            <div class="box-body table-responsive">
              <table id="hotquery" class="table table-hover hide" style="width:100%">
                ...
                <tbody>
                <?php foreach ($hotwordsList['hotwords'] as $id => $hot) { ?>
                <tr>
                  <td><?= $id + 1 ?></td>
                  <td>
                      <?php if ($type && $scriptId) { ?> 
                      <a href="<?= $this->url->get(['for' => 'search-project-script', 'project' => $project, 'type' => $type, 'scriptId' => $scriptId]) ?>?query=<?= $this->escaper->escapeHtml($hot['query']) ?>"> <?= $hot['query'] ?></a>
                      <?php } else { ?>
                      <?= $hot['query'] ?>
                      <?php } ?>
                  </td>
...
{
  var url = "<?= $this->url->get(['for' => 'api-hotwords-update', 'project' => $project]) ?>";
  var payload = {
...
{
  var url = "<?= $this->url->get(['for' => 'api-hotwords-filter', 'project' => $project]) ?>";
  var payload = {
    query:  $( "input[type=hidden][name=query]" ).val()
  }
